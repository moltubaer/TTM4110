...
def user3_generator(env, lambda_rate, Qmin):
    ...
    while True:
        ...
        env.process(user3(env, user_id, Qmin, get_bandwidth(), streaming_type()))
        user_id += 1
        # print(f"Generated User3 Request {user_id} at time {env.now}")

def user3(env, id, Qmin, bandwidth, streaming_type):
    global rejects, successes, k, n, m, used_blocks, bandwidth_modifier, e_total, e_active_user, total_bandwidth, mos, total_mos, gsla_violations, current_level
...
def el_price_simulation(env, pm_h):
    global bandwidth_modifier, e_total, p_total, p_low, p_medium, p_high, rng, current_level
